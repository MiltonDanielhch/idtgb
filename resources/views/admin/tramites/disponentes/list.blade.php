    ...
    <table class="table table-voyager">
        <thead>
            <tr>
                ...
                <th>CI</th>
                <th>Tipo</th>
                <th class="text-center">Fallecimiento</th>
            ...
            @forelse ($data as $item)
                <tr>
                    ...
                    <td class="text-right" style="width: 12%">
                        @can('view', $item)
                            <a href="{{ route('admin.tramites.disponentes.show', [$tramite, $item]) }}" class="btn btn-xs btn-warning" title="Ver">
                                <i class="voyager-eye"></i>
                            </a>
                        @endcan
                        @can('delete', $item)
                            <form action="{{ route('admin.tramites.disponentes.destroy', [$tramite, $item]) }}" method="POST"
                                  style="display: inline-block;" onsubmit="return confirm('¿Quitar disponente?')">
                                @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="6">
                        <h5 class="text-center" style="margin-top: 50px">
                            ...
                            <br><br>
                            No hay disponentes registrados
                        </h5>
                    </td>
                </tr>
            @endforelse
