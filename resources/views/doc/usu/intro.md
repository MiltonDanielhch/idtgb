# 1. Introducción al Sistema IDTGB – Beni

## ¿Qué es el IDTGB?

El **Impuesto Departamental a la Transmisión Gratuita de Bienes (IDTGB)** es un tributo que grava la transferencia de bienes inmuebles en el **departamento del Beni (Bolivia)** cuando se realiza **sin contraprestación económica**, es decir, en casos como:

- Donaciones
- Herencias
- Legados
- Cualquier otro acto jurídico gratuito de transmisión de propiedad

Este impuesto es recaudado por el **Gobierno Autónomo Departamental del Beni** y su tasa varía según el **grado de parentesco** entre quien transmite el bien (disponente) y quien lo recibe (adquirente).

---

## ¿Por qué existe este sistema?

Antes de este sistema, el cálculo del IDTGB se realizaba de forma **manual**, lo que generaba:

- Errores en el cálculo del impuesto
- Tiempos de espera prolongados
- Falta de transparencia en la aplicación de tasas y exenciones
- Dificultad para verificar la autenticidad de los formularios

El **Sistema IDTGB – Beni** nace para **modernizar, agilizar y garantizar la transparencia** en este trámite, ofreciendo:

- Un **cálculo automático y preciso** del impuesto, actualizado con valores UFV y tasas vigentes.
- Un **formulario oficial digital (A-01)** con medidas de seguridad como códigos QR.
- Una **calculadora pública** para que los ciudadanos obtengan estimaciones rápidas.

---

## ¿Quiénes lo usan?

El sistema tiene dos tipos de usuarios:

### 👤 Ciudadanos (público en general)
- **Acceso público y sin registro** a una calculadora para obtener una **estimación** del impuesto.
- Pueden informarse sobre los requisitos y montos aproximados antes de iniciar el trámite.
- Verifican la autenticidad de un formulario oficial escaneando su código QR.

### 👨‍💼 Funcionarios autorizados (administradores)
- Gestionan trámites completos desde el **panel de administración**.
- Registran personas, inmuebles, avalúos, documentos y pagos.
- Supervisan la aplicación de **exenciones** (como la de vivienda única).
- Emiten formularios oficiales y envían reportes al **Sistema de Impuestos Nacionales (SIN)**.

---

## Beneficios del sistema

✅ **Cálculo automático del impuesto**  
El sistema aplica las tasas correctas según el parentesco, descuentos (15 % si aplica), recargos por mora y ajustes por UFV, sin intervención manual.

✅ **Transparencia total**  
Todos los pasos del trámite quedan registrados. Las exenciones y montos se muestran claramente.

✅ **Seguridad y Validez**  
Cada Formulario A-01 oficial incluye un **código QR único** que permite verificar su autenticidad en tiempo real, evitando falsificaciones.

✅ **Listo para integración nacional**  
Al finalizar un trámite, el sistema genera un archivo estandarizado y lo envía automáticamente al **SIN**, cumpliendo con los requisitos legales.

---
